    ...
    function Window:AddTab(Name)
        ...
        local function FadeTab(Frame, Show)
            ...
            for _, Desc in next, Frame:GetDescendants() do
                if Desc:IsA('TextLabel') or Desc:IsA('TextBox') then
                    local Stored = Desc:GetAttribute('_TabTextT');
                    if Stored == nil then
                        Stored = Desc.TextTransparency;
                        Desc:SetAttribute('_TabTextT', Stored);
                    end;

                    if Show then
                        Desc.TextTransparency = 1;
                        Library:Tween(Desc, { TextTransparency = Stored }, Duration * 0.85, Enum.EasingStyle.Quad, Enum.EasingDirection.Out);
                    else
                        Library:Tween(Desc, { TextTransparency = 1 }, Duration * 0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.In);
                    end;
                end;
            end;
        end;
        ...
        function Tab:HideTab()
            ...
            if TabFrame.Visible then
                FadeTab(TabFrame, false);
                task.delay(Library.Anim.Tab * 0.45, function()
                    if TabButtonLabel.TextTransparency > 0.2 then
