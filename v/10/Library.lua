...
-- Top accent that follows the rounded corner curve (rim + cover), not a flat line that sticks out.
function Library:AddAccentBar(Parent, Radius, ZIndex)
    ...
    -- Covers the rim except the top strip; shared UICorner makes the accent arc with the shell.
...
-- Rounded shell with a soft AA stroke under the outline (hides UICorner stair-steps).
function Library:ApplyRound(Parent, Radius, StrokeColorKey, WithShadow)
    ...
    -- Soft feather matching the fill color — masks jagged corner pixels.
    local Soft = Parent:FindFirstChild('RoundSoft');
    if not Soft then
        Soft = Library:Create('UIStroke', {
            Name = 'RoundSoft';
            Color = Parent.BackgroundColor3;
            Thickness = 1.6;
            Transparency = 0.45;
        ...
        Soft.Color = Parent.BackgroundColor3;
        Soft.Thickness = 1.6;
        Soft.Transparency = 0.45;
        Soft.LineJoinMode = Enum.LineJoinMode.Round;
    end;

    local SoftKey = 'MainColor';
    if Parent.BackgroundColor3 == Library.BackgroundColor then
        SoftKey = 'BackgroundColor';
    elseif Parent.BackgroundColor3 == Library.MainColor then
        SoftKey = 'MainColor';
    end;
    ...
    local ParentReg = Library.RegistryMap[Parent];
    if ParentReg and ParentReg.Properties and ParentReg.Properties.BackgroundColor3 then
        SoftKey = ParentReg.Properties.BackgroundColor3;
    end;

    Library:AddToRegistry(Soft, {
        Color = SoftKey;
    });
...
-- Ghost the window while dragging/resizing. Does NOT touch the menu blur.
function Library:SetDragVisual(Instance, Enabled)
    local Ghost = Library.Anim.DragGhost or 0.28;
    ...
    if Instance:IsA('CanvasGroup') then
        ...
        Library:Tween(Instance, {
            BackgroundTransparency = Enabled and (Ghost * 0.55) or 0;
        }, 0.14, Enum.EasingStyle.Quad, Enum.EasingDirection.Out);
    end;
end;
    ...
    function Funcs:AddColorPicker(Idx, Info)
        ...
        function ColorPicker:Hide()
            ...
            task.delay(Library.Anim.ColorPicker * 0.75, function()
                if PickerFrameOuter:GetAttribute('HideToken') == Token and not Library.OpenedFrames[PickerFrameOuter] then
...
do
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateBaseButton(Button)
            ...
            Outer.MouseEnter:Connect(function()
                if not Outer:GetAttribute('Pressed') then
                    Library:Tween(Scale, { Scale = 1.02 }, Library.Anim.Button);
                end;
            end);

            Outer.MouseLeave:Connect(function()
                Outer:SetAttribute('Pressed', false);
                Library:Tween(Scale, { Scale = 1 }, Library.Anim.Button);
            end);

            Outer.InputBegan:Connect(function(Input)
                if Input.UserInputType == Enum.UserInputType.MouseButton1 then
                    Outer:SetAttribute('Pressed', true);
                    Library:Tween(Scale, { Scale = 0.96 }, Library.Anim.Button * 0.75);
                end;
            end);
    ...
    function Funcs:AddDropdown(Idx, Info)
        ...
        -- Single outer shell so open dropdown reads as one connected rounded box.
        local ConnectShell = Library:Create('Frame', {
            BackgroundColor3 = Library.MainColor;
            BorderSizePixel = 0;
            Visible = false;
            ZIndex = 19;
        ...
        Library:ApplyRound(ConnectShell, 6, 'OutlineColor');
        Library:AddToRegistry(ConnectShell, {
            BackgroundColor3 = 'MainColor';
        });
        ...
        -- No stroke on the list — ConnectShell owns the outline.
        Library:ApplyRound(ListOuter, 6, false);
        ...
        local function GetDropdownScreenPos()
            local Pos = DropdownOuter.AbsolutePosition;
            local Size = DropdownOuter.AbsoluteSize;
            return Pos.X, Pos.Y, Size.X, Size.Y;
        end;

        local function RecalculateListPosition()
            local X, Y, W, H = GetDropdownScreenPos();
            local ListH = (ListOuter.Visible and not ListOuter:GetAttribute('Closing')) and DropdownListHeight or ListOuter.Size.Y.Offset;

            ConnectShell.Position = UDim2.fromOffset(X, Y);
            ConnectShell.Size = UDim2.fromOffset(W, H + math.max(ListH, 0));
        ...
        function Dropdown:OpenDropdown()
            ...
            ConnectShell.Visible = true;
            Library.OpenedFrames[ListOuter] = true;
            Library.OpenedFrames[ConnectShell] = true;
            ...
            if ListCorner then
                -- Only the bottom of the combined shell should read as rounded.
            ...
            local X, Y, _, H = GetDropdownScreenPos();
            ConnectShell.Position = UDim2.fromOffset(X, Y);
            ConnectShell.Size = UDim2.fromOffset(W, H);
            Library:Tween(ConnectShell, {
                Size = UDim2.fromOffset(W, H + DropdownListHeight);
            }, Library.Anim.Dropdown, Enum.EasingStyle.Quint, Enum.EasingDirection.Out);
        ...
        function Dropdown:CloseDropdown()
            ...
            Library.OpenedFrames[ConnectShell] = nil;
...
function Library:CreateWindow(...)
    ...
    -- Fullscreen dim behind the menu (pairs with the Lighting blur).
    local MenuDim = Library:Create('Frame', {
        BackgroundColor3 = Color3.new(0, 0, 0);
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
        Size = UDim2.fromScale(1, 1);
    ...
    local function LayoutTitleBar()
        local StatusWidth = math.ceil(StatusLabel.TextBounds.X);
        if StatusWidth < 1 and StatusLabel.Text ~= '' then
            StatusWidth = select(1, Library:GetTextBounds(StatusLabel.Text, Library.Font, 16));
        end;

        -- Keep the title optically centered while reserving matching space on both sides
        -- so it never runs under the status text.
        local Side = math.max(StatusWidth + 12, 8);
        WindowLabel.Position = UDim2.new(0, Side, 0, 0);
        WindowLabel.Size = UDim2.new(1, -(Side * 2), 0, 25);
        StatusLabel.Position = UDim2.new(1, -8, 0, 0);
    end;

    StatusLabel:GetPropertyChangedSignal('Text'):Connect(LayoutTitleBar);
    StatusLabel:GetPropertyChangedSignal('TextBounds'):Connect(LayoutTitleBar);
    ...
    if not Config.Status then
        task.spawn(function()
            ...
            if Success and Info and Info.Name then
                StatusLabel.Text = string.format('[UP] %s', Info.Name);
            else
                StatusLabel.Text = string.format('[UP] %s', game.Name or 'Unknown');
            end;
    ...
    -- One shared sliding pill — replaces the old per-tab square accent bar.
    ...
    local function MoveTabSlider(Button, Instant)
        ...
        local GoalPos = UDim2.new(0, RelX + 5, 1, -3);
    ...
    function Window:AddTab(Name)
        ...
        local function SlideTab(Frame, Show, Direction)
            local Duration = Library.Anim.Tab;
            local Offset = 28;

            if Show then
                Frame.Visible = true;
                Frame.Position = UDim2.new(0, Direction * Offset, 0, 0);
                Library:Tween(Frame, {
                    Position = UDim2.new(0, 0, 0, 0);
                }, Duration, Enum.EasingStyle.Quint, Enum.EasingDirection.Out);
            else
                Library:Tween(Frame, {
                    Position = UDim2.new(0, -Direction * Offset, 0, 0);
                }, Duration * 0.85, Enum.EasingStyle.Quint, Enum.EasingDirection.In);
            end;
        end;

        function Tab:ShowTab()
            local From = Window.ActiveTabIndex or 0;
            local Direction = (Tab.Index >= From) and 1 or -1;

            for _, OtherTab in next, Window.Tabs do
                if OtherTab ~= Tab then
                    OtherTab:HideTab(Direction);
                end;
            end;

            Library:Tween(TabButtonLabel, { TextTransparency = 0 }, Library.Anim.Tab * 0.6, Enum.EasingStyle.Quad, Enum.EasingDirection.Out);
            MoveTabSlider(TabButton, not TabSlider.Visible);
            Window.ActiveTabIndex = Tab.Index;
            SlideTab(TabFrame, true, Direction);
        end;

        function Tab:HideTab(Direction)
            Direction = Direction or 1;
            Library:Tween(TabButtonLabel, { TextTransparency = 0.35 }, Library.Anim.Tab * 0.45, Enum.EasingStyle.Quad, Enum.EasingDirection.Out);

            if TabFrame.Visible then
                local Token = (TabFrame:GetAttribute('SlideToken') or 0) + 1;
                TabFrame:SetAttribute('SlideToken', Token);
                SlideTab(TabFrame, false, Direction);

                task.delay(Library.Anim.Tab * 0.85, function()
                    if TabFrame:GetAttribute('SlideToken') == Token and Window.ActiveTabIndex ~= Tab.Index then
        ...
        function Tab:AddTabbox(Info)
            ...
            -- No full-width accent here — each nested tab owns its own top + bottom bars.
            ...
            function Tabbox:AddTab(Name)
                ...
                -- Top + bottom bars on every nested tab (active = accent, idle = outline).
                ...
                local function SetBars(Active)
                    local Color = Active and Library.AccentColor or Library.OutlineColor;
                    local Key = Active and 'AccentColor' or 'OutlineColor';
                    TopBar.BackgroundColor3 = Color;
                    BottomBar.BackgroundColor3 = Color;
                    Library.RegistryMap[TopBar].Properties.BackgroundColor3 = Key;
                    Library.RegistryMap[BottomBar].Properties.BackgroundColor3 = Key;
                end;
    ...
    function Library:Toggle()
        ...
        if Toggled then
            ...
            Library:Tween(MenuDim, { BackgroundTransparency = 0.45 }, FadeTime, Enum.EasingStyle.Linear);
            ...
            Library:Tween(MenuDim, { BackgroundTransparency = 1 }, FadeTime, Enum.EasingStyle.Linear);
        end;

        -- Stock Linoria black fade (per-descendant transparencies).
        ...
        -- Fade the outer shell itself too.
        do
            local Cache = TransparencyCache[Outer];
            if not Cache then
                Cache = { BackgroundTransparency = Outer.BackgroundTransparency };
                TransparencyCache[Outer] = Cache;
            end;
            if Cache.BackgroundTransparency == nil then
                Cache.BackgroundTransparency = 0;
            end;
            TweenService:Create(Outer, TweenInfo.new(FadeTime, Enum.EasingStyle.Linear), {
                BackgroundTransparency = Toggled and Cache.BackgroundTransparency or 1;
            }):Play();
        end;
